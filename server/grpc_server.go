...
func NewAgentServer(chat llm.Chat, db *storage.DB) *AgentServer {
	server := &AgentServer{
		chat:        chat,
		db:          db,
...
func (s *AgentServer) AddMCPService(ctx context.Context, req *pb.MCPServiceRequest) (*pb.MCPServiceResponse, error) {
	s.mcpLock.Lock()
	...
	if s.db != nil {
		...
		if err := s.db.CreateMCPService(dbService); err != nil {
...
func (s *AgentServer) RemoveMCPService(ctx context.Context, req *pb.MCPServiceRequest) (*pb.MCPServiceResponse, error) {
	s.mcpLock.Lock()
	defer s.mcpLock.Unlock()
	...
	// 从数据库中删除
	if s.db != nil {
		if err := s.db.DeleteMCPServiceByName(req.Name); err != nil {
...
func (s *AgentServer) ListMCPServices(ctx context.Context, req *pb.Empty) (*pb.MCPServicesResponse, error) {
	s.mcpLock.RLock()
...
func (s *AgentServer) mcpServiceInfoToProto(info *MCPServiceInfo) *pb.MCPServiceInfo {
...
func (s *AgentServer) createExecutor(req *pb.ChatRequest, ctx *agent.Context) (*agent.AgentExecutor, error) {
	...
	// 如果有系统提示词，添加到内存
	if systemPrompt != "" {
		ctx.GetMemory().AddMessage(core.Message{
			Role:    core.MessageRoleSystem,
